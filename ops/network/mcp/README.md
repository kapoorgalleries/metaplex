...
Requirements on the admin machine: Node 20+, `ssh`/`scp` on PATH, and bash for the kit's `.sh` scripts. macOS and Linux have it. On Windows install Git for Windows (`winget install Git.Git`): the server looks for `Program Files\Git\bin\bash.exe`, then for `bin\bash.exe` next to `git --exec-path`, and never uses a bare `bash` (a default Git install does not put it on PATH, and `System32\bash.exe` is WSL, which cannot run the kit); set `TRIMURTI_BASH` if Git lives elsewhere. `trimurti_scan_lan` uses `netscan.ps1` there.

The admin key must already exist and be on every host: that is `scripts/ssh-keys.sh`, run once from a terminal, because the first push asks for each host's password and MCP has no terminal. If the key has a passphrase, it must be loaded in an ssh-agent that the MCP server can see (the client that starts the server must inherit `SSH_AUTH_SOCK`; on macOS the Keychain is tried). Otherwise every SSH tool stops before contacting any host with `key is passphrase-protected and no ssh-agent holds it: run ssh-add <path>`.
...
| `trimurti_upsert_host` | no | Create a row, or change only the fields given (`{name, trimurti: "yes"}` changes nothing else). Strictly validated: IPv4 without leading zeros, MAC, enums, users not starting with `-`, no commas. New names are stored lowercase. Warns when an IP is on two rows |
...
| `trimurti_ssh_run` | no | One command on one inventory host (`default`, `bash` or `powershell` shell), output capped (head and tail kept). A `hint` only when ssh itself failed or the command timed out; on Linux and macOS hosts with GNU `timeout` a timed-out command is stopped on the host |
| `trimurti_run_script` | no | Push `bootstrap-ai-clis`, `update-all`, `enable-ssh-server` or `disk-triage` to the selected hosts as a background job. `--harden` is refused (it needs Sanjay's yes and a terminal) |
...
- `trimurti_ssh_run` is the one tool that can run anything on a machine; it is annotated destructive so clients ask. `trimurti_remove_host` is annotated destructive too. `trimurti_run_script` is limited to the four kit scripts, none of which formats, wipes or reboots, and it refuses `--harden`.
