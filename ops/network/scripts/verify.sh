...
# the versions of claude, codex, gemini, hf and node. Prints a table and saves it
...
for row in $HOSTS; do
  ...
  claude=; codex=; gemini=; hf=; node=; osname=; sshok="NO"
  ...
  printf '| %s | %s | %s | %s | %s | %s | %s | %s | %s |\n' "$name" "$p" "$sshok" "${claude:--}" "${codex:--}" "${gemini:--}" "${hf:--}" "${node:--}" "${osname:--}" | tr -d '\r' >> "$MD"
  IFS='
'
done
IFS="$OLDIFS"
